...
The **phase boundary** is the gap between two phases, and it is the only place this decision belongs. Mid-phase there is no decision to make: continue, or isolate independent lookups with parallel tool calls. Compacting mid-phase makes the agent lose the thread.
...
| **Parallel tools** | Run independent lookups concurrently without leaving the session. |
...
**4. Can independent research run without steering?** If so, use Pi's parallel tool calls and keep the current session intact. If the active Pi environment exposes an explicit background-agent tool, it may be used; otherwise perform the work in the current session.
