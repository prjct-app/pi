...
test('completion requires a native observation; a host-recorded bash outcome closes the loop', async () => {
  ...
  try {
    ...
    await runtime.execute('prjct_task', { action: 'claim', workId, taskId: 'task_test',
      checkoutId, access: 'write', operationId: 'op_c', expectedRevision: await rev(), maxBytes: 4096 });
    ...
    await second.execute('prjct_task', { action: 'claim', workId, taskId: 'task_next', checkoutId, access: 'write', operationId: 'op_c2', expectedRevision: await rev(), maxBytes: 4096 });
